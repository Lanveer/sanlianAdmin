...
class CompetitionBallteamModel extends Model{
    protected $_validate = [
    ...
    public function getById($id){
        $result = $this->where("ball_team_id=%d",$id)->find();
        if(empty($result)){
            $result = [];
        }
        return $result;
    ...
    public function getByOrderId($id){
        $data = $this
            ->alias("cb")
            ->field("cb.*,c.title AS competition_title,b.name as ball_team_name")
            ->join("left join t_competition c on cb.competition_id=c.competition_id")
            ->join("left join t_ball_team b on cb.ball_team_id=b.ball_team_id")
            ->where("order_id=%d",$id)
            ->find();
        if(empty($data)){
            $data = [];
    ...
    public function updateBallTeam($input){
    ...
    public function delById($id){
        ...
        $result = $this->where("ball_team_id=%d",$id)->delete();
        return $result;
    }
     ...
     * @throws \Exception
     */
    public function addBallTeam($input){
        $data = $this->create($input);
        if(!$data){
            throw new \Exception($this->getError());
        }else{

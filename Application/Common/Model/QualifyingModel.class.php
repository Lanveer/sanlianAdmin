...
class QualifyingModel extends Model{
    protected $_validate = [
    ...
    public function search($queryParams){
        ...
        $sort = $queryParams['sort']?$queryParams['sort']:'qualifying_id';
    ...
    public function getByOrderId($order_id){
        $condition['_string'] = "(home_order_id={$order_id} OR guest_order_id={$order_id})";
        $data = $this
            ->alias("q")
            ->join('t_court AS c on q.court_id=c.court_id')
            ->where($condition)
            ->find();
    }
    ...
    public function getById($id){
        $result = $this->where("qualifying_id=%d",$id)->find();
        if(empty($result)){
            $result = [];
        }
        return $result;
    ...
    public function updateQualifying($input){
    ...
    public function delById($id){
        ...
        $result = $this->where("qualifying_id=%d",$id)->delete();
        return $result;
    }
     ...
     * @throws \Exception
     */
    public function addQualifying($input){
        $data = $this->create($input);
        if(!$data){
            throw new \Exception($this->getError());
        }else{
